...
def seed_data():
    ...
    """
    frappe.logger().info("Candela: Running seed_data()...")

    _seed_settings()
    _seed_roles()

    frappe.db.commit()
    frappe.logger().info("Candela: seed_data() complete.")
...
def _seed_settings():
    ...
    settings_dt = None
    for dt_name in ["CD Settings", "Candela Settings", "Candela Settings"]:
...
def _seed_roles():
    ...
    roles = [
        {"role": "CD User", "desk_access": 1},
        {"role": "CD Manager", "desk_access": 1},
        {"role": "CD Admin", "desk_access": 1},
    ]
