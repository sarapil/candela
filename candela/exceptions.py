"""
Candela — Custom Exception Hierarchy
...
class CandelaError(Exception):
    """Candela base exception — all app-specific errors inherit from this."""

    def __init__(self, message=None, title=None):
        self.message = message or frappe._("An error occurred in Candela")
        self.title = title or frappe._("Candela Error")
        super().__init__(self.message)


class ValidationError(CandelaError):
...
class NotFoundError(CandelaError):
...
class PermissionError(CandelaError):
...
class ConfigurationError(CandelaError):
...
class IntegrationError(CandelaError):
...
class RateLimitError(CandelaError):
