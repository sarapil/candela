# Contributing to Candela — المساهمة في Candela
...
```bash
git clone https://github.com/YOUR_USERNAME/candela.git
cd candela
```
...
```bash
bench get-app candela /path/to/your/fork
bench --site dev.localhost install-app candela
...
bench --site dev.localhost run-tests --app candela

# Run specific test — تشغيل اختبار محدد
bench --site dev.localhost run-tests --app candela --module candela.tests.unit.test_my_service
```
...
- Update help files in `candela/help/` for DocType changes
